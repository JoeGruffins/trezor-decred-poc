...
const uiActions = {
    ...
    purchaseTicket: async () => {
      ...
      const address_n = trezorHelpers.addressPath(0, 0);
      /*
      const resp = await session.getAddress({
        path: address_n,
        coin: coin,
      ...
      const addr = resp.payload.address;
      */
      const addr = "SspR5D7xL2XmyDkQxSfHJQBpxh7kwwwZyuK"
      const zeroAddr = "SsUMGgvWLcixEeHv3GT4TGYyez4kY79RHth"

      //const ticketInputTxHash = await sendToAddr(addr, totalPrice)
      //log(ticketInputTxHash)
      const ticketInputTxHash = "bb227c2a17fee03756fb12e10e2e50e97fa7216556b812801493bcfa52f94bd1"
      const inp = await wallet.getTransaction(wsvc, ticketInputTxHash)
      ...
      const opreturn = trezorHelpers.makeCommitmentPush(addr, totalPrice)
      ...
      const inputs = [input]
      const outputs = [sstxsubmission, sstxcommitment, sstxchange]
      ...
      // const poolInput = await sendToAddr(addr, poolFee)
